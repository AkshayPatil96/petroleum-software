...
export default function Reports({ bills, settings }: { bills: Bill[], settings: PumpSettings }) {
  ...
  const filteredBills = bills.filter(b => {
    ...
    // Time filter
    let timeMatch = true;
    if (filter === 'today') timeMatch = format(date, 'yyyy-MM-dd') === format(now, 'yyyy-MM-dd');
    else if (filter === 'week') timeMatch = (now.getTime() - date.getTime()) < 7 * 24 * 60 * 60 * 1000;
    ...
    <div className="space-y-6">
            ...
            {(['today', 'week', 'month'] as const).map(f => (
              <button 
                ...
                className={cn(
                  ...
                  filter === f ? "bg-slate-900 text-white shadow-sm" : "text-slate-600"
                )}
              >
                {f}
              </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Card className="p-6 bg-slate-900 text-white">
          <p className="text-slate-400 text-sm font-medium">Total Volume Sold</p>
          <h3 className="text-3xl font-bold mt-1">{totalQuantity.toFixed(2)} Units</h3>
        </Card>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase">Pump / Nozzle</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredBills.length > 0 ? filteredBills.map(bill => (
                <tr key={bill.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 font-mono text-xs font-bold text-blue-600">{bill.id}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{format(new Date(bill.date), 'dd MMM, HH:mm')}</td>
                  <td className="px-6 py-4 font-bold text-slate-900">{bill.vehicleNumber || 'N/A'}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">
                    {(bill.pumpNumber || 'N/A')} / {(bill.nozzleNumber || 'N/A')}
                  </td>
                  <td className="px-6 py-4">
                    <span className="px-2 py-1 bg-slate-100 rounded text-xs font-bold">{bill.fuelType}</span>
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-600 text-right">{bill.quantity.toFixed(2)} {bill.unit === 'Liter' ? 'L' : bill.unit === 'Kg' ? 'Kg' : 'Pcs'}</td>
                  <td className="px-6 py-4 font-bold text-slate-900 text-right">{formatCurrency(bill.totalAmount)}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            ...
            >
              <div className="p-8 space-y-6">
                ...
                <ReceiptContent bill={selectedBill} settings={settings} />

                <div className="flex gap-3 no-print">
                  <Button variant="outline" className="flex-1" onClick={() => setSelectedBill(null)}>
                    Close
                  </Button>
            ...
            <div className={cn(
              ...
              <div className="border-t border-b border-black py-2 my-2">
                ...
                <p>DATE: {format(new Date(selectedBill.date), 'dd/MM/yyyy HH:mm')}</p>
                {settings.visibleFields.vehicleNumber && <p>VEHICLE: {selectedBill.vehicleNumber}</p>}
                {settings.visibleFields.pumpNumber && selectedBill.pumpNumber && <p>PUMP: {selectedBill.pumpNumber}</p>}
                {settings.visibleFields.nozzleNumber && selectedBill.nozzleNumber && <p>NOZZLE: {selectedBill.nozzleNumber}</p>}
                {settings.visibleFields.customerName && selectedBill.customerName && <p>CUSTOMER: {selectedBill.customerName}</p>}
              </div>
              <table className="w-full text-left my-2">
                ...
                <tbody>
                  <tr>
                    <td>{selectedBill.fuelType} @ {selectedBill.pricePerLiter.toFixed(2)}</td>
                    <td className="text-right">{selectedBill.quantity.toFixed(2)} {selectedBill.unit === 'Liter' ? 'L' : selectedBill.unit === 'Kg' ? 'Kg' : 'Pcs'}</td>
                    <td className="text-right">{selectedBill.totalAmount.toFixed(2)}</td>
                  </tr>
                </tbody>
              </table>
              <div className="border-t border-black pt-2 mt-2 text-right">
                <p className="font-bold text-lg">TOTAL: {formatCurrency(selectedBill.totalAmount)}</p>
                {settings.visibleFields.paymentMode && <p className="text-xs">Payment: {selectedBill.paymentMode}</p>}
              </div>
              <div className="text-center mt-6">
                {settings.visibleFields.footerNote && selectedBill.customNote && <p className="mb-2">{selectedBill.customNote}</p>}
                <p>*** THANK YOU ***</p>
