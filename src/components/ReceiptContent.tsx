...
export default function ReceiptContent({ bill, settings, isLivePreview = false }: { bill: Partial<Bill>, settings: PumpSettings, isLivePreview?: boolean }) {
  ...
  const unitLabel = unit === 'Liters' ? 'L' : unit === 'Kg' ? 'Kg' : 'Pcs';

  return (
    <div className={cn(
      "text-left font-mono text-sm",
      isLivePreview ? "scale-90 origin-top" : "",
      settings.receiptTemplate === 'Compact' ? "text-xs" : 
      settings.receiptTemplate === 'Modern' ? "bg-white border border-slate-200 rounded-none border-t-4 border-t-blue-600 p-6" : 
      ...
      <div className={cn(
        ...
        {settings.logoUrl && (
          <img src={settings.logoUrl} alt="Logo" className="h-10 w-auto object-contain mx-auto mb-2" referrerPolicy="no-referrer" />
        )}
        <h4 className="font-bold text-lg uppercase tracking-tight leading-tight">{settings.name}</h4>
        <p className="text-[10px] text-slate-500 leading-tight">{settings.address}</p>
        <div className="flex flex-wrap justify-center gap-x-4 gap-y-1 mt-2">
          {settings.visibleFields.gstin && settings.gstin && <p className="text-[9px] text-slate-500">GSTIN: {settings.gstin}</p>}
          {settings.visibleFields.licenseNo && settings.licenseNo && <p className="text-[9px] text-slate-500">LIC: {settings.licenseNo}</p>}
        </div>
      </div>
      ...
      <div className="space-y-1">
        ...
        {settings.visibleFields.customerName && customerName && (
          <div className="flex justify-between">
            <span>Customer:</span>
            <span className="font-bold truncate max-w-[120px]">{customerName}</span>
          </div>
        )}
        <div className="flex justify-between">
          <span>Item:</span>
          <span>{fuelType} @ ₹{pricePerLiter.toFixed(2)}</span>
        </div>
        <div className="flex justify-between">
          <span>Quantity:</span>
          <span className="font-bold">{quantity.toFixed(3)} {unitLabel}</span>
        </div>
      </div>
      ...
      <div className="mt-2 space-y-1">
        ...
        <div className="flex justify-between text-[10px] text-slate-500">
          <span>Status:</span>
          <span className={cn("font-bold", status === 'Draft' ? "text-amber-500" : "text-emerald-500")}>
            {status}
          </span>
        </div>
      </div>
